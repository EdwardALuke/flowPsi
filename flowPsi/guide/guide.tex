...
\item {\tt plot\_output}
  ...
  along with the default variables.  A list of the additional variables
...
\item {\tt plot\_output\_exclusive}
  ...
  \ref{tab:extract} table \ref{tab:nodalextract}.  Note that if you
...
in the {\tt .vars} file.  The scalar variables that can be optionally written
by {\em FlowPsi} are given in Table \ref{tab:nodalextract}, and the vector
variables are given in Table \ref{tab:vectorextract}.  For example, to
extract the turbulence model scalars {\tt k}, {\tt w}, and {\tt tmuu},
one would place the following in the {\tt .vars} file:
\begin{verbatim}
// Note, no spaces between variables, just commas
plot_output: k,w,tmuu
\end{verbatim}
